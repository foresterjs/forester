...
var tokenCheckMiddleware = require('../defender-middlewares/token-check');
  ...
  rest.registerEndpoint(
    {
      ...
      route: "/",
      middlewares: [
        wrap(tokenCheckMiddleware(rest.app)),
        wrap(commonMiddlewares.showSchema({collection})),
        wrap(commonMiddlewares.showCount({collection})),
        wrap(commonMiddlewares.showPagesCount({collection})),
        wrap(findAll({collection}))],
